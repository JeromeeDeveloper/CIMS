...
class UserController extends Controller
{
    ...
    public function changePasswordPage(Request $request, $id)
    {
        if($request->session()->has('passworduser_id'))
        {
            ...
            {
                $currentTime = time();
                if($currentTime > $request->session()->get('passwordtime')) //20 Minutes
                {
        ...
        else
        {
            if($this->checkEmail($request->email))
            {
                ...
                if($user->role < 3) //means only user (Admin and staff)
                {
                    ...
                    $request->session()->put('passwordtime', time() + (1 * 600));
                    $status = 1;
                }
                else
                {
    ...
    public function data()
    {
        ...
        return datatables()->of($users)
                        ...
                        ->addColumn('action', function ($row) {
                            $html = "";
                            if(Auth::user()->id == $row->id && Auth::user()->id > 1) // Staff
                            {
